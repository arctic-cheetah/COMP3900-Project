...
class preprocess_data:
    ...
    def _fetch_html_playwright(self, url: str) -> str:
        """Notebook-safe sync wrapper around async Playwright fetching."""
        try:
            asyncio.get_running_loop()
        except RuntimeError:
            return asyncio.run(self._fetch_html_playwright_async(url))
        ...
        def _runner() -> None:
            try:
                result["html"] = asyncio.run(self._fetch_html_playwright_async(url))
    ...
    async def _fetch_html_playwright_async(self, url: str) -> str:
        """
        ...
        async with async_playwright() as pw:
            ...
            try:
                ...
                try:
                    await page.wait_for_selector("input", timeout=timeout_ms / 10)
                except Exception:
                    pass
