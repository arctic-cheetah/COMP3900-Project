...
UCI_PATH = "backend/ml/data/uci_phishing_url_dataset_clean.csv"
STEALTH_PATH = "backend/ml/data/StealthPhisher2025.csv"
OUTPUT_PATH = "backend/ml/data/uci_phishing_url_dataset_clean_stealth2025.csv"
...
def main() -> None:
    ...
    stealth_aligned = pd.DataFrame(columns=uci_cols)
    for col in uci_cols:
        if col in stealth_df.columns:
    ...
    merged = pd.concat([uci_df, stealth_aligned], ignore_index=True)
    ...
    mapped_uci_cols = set(STEALTH_TO_UCI.values()) | set(PREPROCESSOR_FILL_COLS)
    missing_uci_cols = [col for col in uci_cols if col not in mapped_uci_cols]
    if missing_uci_cols:
        print("UCI columns without Stealth match:")
        print(", ".join(missing_uci_cols))

    unused_stealth_cols = [
        col for col in stealth_df.columns if col not in uci_cols and col != "IsLegit"
    ]
